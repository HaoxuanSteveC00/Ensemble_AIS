...
log_weight_AIS1 = delta_t.*(0.5.*(power(vecnorm(x_values_AIS1-...
    ...
    log(oneD_GL_PDF(x_values_AIS1,delta, beta, h)));
log_weight_AIS2 = delta_t.*(0.5.*(power(vecnorm(x_values_AIS2-...
    start_center_mat,2,2), 2.0)./sigmas_init) +...
    log(oneD_GL_PDF(x_values_AIS2, delta, beta, h)));
...
for i = 1:N_steps
    ...
    for j = 1:N_samples
        ...
        drive = -T.*oneD_GL_grad(x_values(j,:), delta, beta, h) +...
            (1.0 - T).*((x_values(j,:) - start_center)./sigmas_init);
        x_j_prop = x_values(j,:) - delta_t.*drive +...
            sqrt(2.*delta_t).*noise;

        drive_new = -T.*oneD_GL_grad(x_j_prop, delta, beta, h) +...
            (1.0 - T).*((x_j_prop - start_center)./(sigmas_init));
        prob_f = power(oneD_GL_PDF(x_j_prop, delta, beta, h), T).*...
        ...
        prob_s = power(oneD_GL_PDF(x_values(j,:), delta, beta, h), T).*...
        ...
        prob1 = power(oneD_GL_PDF(new_point, delta, beta, h), T);
        prob2 = power(oneD_GL_PDF(x_values(j,:), delta, beta, h), T);
        ...
        % Birth Death
        rate = -log(oneD_GL_PDF(x_values, delta, beta, h)) -...
    ...
    for j = 1:N_samples
        ...
        % MALA
        noise = mvnrnd(zeros(dim,1),eye(dim));
        drive = -T.*oneD_GL_grad(x_values_c(j,:), delta, beta, h) +...
        ...
        move_f = exp(-power(norm(x_j_prop - x_values_c(j,:) +...
            delta_t.*drive), 2.0)/(4*delta_t));
        prob_s = power(oneD_GL_PDF(x_values_c(j,:), delta, beta, h), T)...
            .*power(exp(-power(norm(x_values_c(j,:) - start_center),...
        ...
        % Birth Death
        rate = -log(oneD_GL_PDF(x_values_c, delta, beta, h)) -...
    ...
    loss_table(i) = -mean(log(oneD_GL_PDF(x_values, delta, beta, h)))...
        - log(N_samples);
    loss_table_c(i) = -mean(log(oneD_GL_PDF(x_values_c, delta, beta,...
        h))) - log(N_samples);
    ...
    for j = 1:N_samples
        for k = 1:inner_iter_AIS
            noise = mvnrnd(zeros(dim,1),eye(dim));
            drive = -T.*oneD_GL_grad(x_values_AIS1(j,:), delta, beta,...
            ...
            move_f = exp(-power(norm(x_j_prop - x_values_AIS1(j,:) +...
                delta_t.*drive), 2.0)/(4*delta_t));
            prob_s = power(oneD_GL_PDF(x_values_AIS1(j,:), delta, beta,...
                h), T).*power(exp(-power(norm(x_values_AIS1(j,:) -...
                start_center), 2.0) /(2.0*sigmas_init)), 1.0 - T);
            ...
            log_weight_AIS1(j) = log_weight_AIS1(j) +...
                ...
                2.0))./sigmas_init + log(oneD_GL_PDF(pt_new,...
                delta, beta, h)));
        end    
    end    
    ...
    for j = 1:N_samples
        ...
        for k = 1:inner_iter_AIS
            ...
            prob_f = power(oneD_GL_PDF(x_j_new, delta, beta, h), T).*...
                power(exp(-power(norm(x_j_new - start_center), 2.0)...
                / (2.0*sigmas_init)), 1.0 - T);
            prob_s = power(oneD_GL_PDF(pt, delta, beta, h), T).*...
